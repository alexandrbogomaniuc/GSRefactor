...
class GameRebuyDialogController extends GameBaseDialogController
{
	...
	{	
			...
			&& APP.battlegroundController.info.isBattlegroundGameStarted)
		{
			...
			this.emit(GameRebuyDialogController.EVENT_ON_REBUY_DIALOG_EXIT_CLICKED);
			this.__deactivateDialog();
		}
		else 
		{
		...
		switch (msgType)
		{
			case GAME_MESSAGES.SERVER_ERROR_MESSAGE_RECIEVED:
				if (
						...
						&& event.data.errorCode == LobbyWebSocketInteractionController.ERROR_CODES.BUYIN_NOT_ALLOWED_AT_CURRENT_GAME_STATE
					)
				{
