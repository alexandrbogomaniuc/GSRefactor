...
import LobbyPendingOperationInfo from '../../model/gameplay/LobbyPendingOperationInfo';
...
class LobbyPendingOperationController extends GUPendingOperationController
{
	...
	{
		super(aOptInfo_poi || new LobbyPendingOperationInfo);
	}
	...
	__initControlLevel()
	{
		...
		if (this.info.isPendingOperationHandlingSupported)
		{
			...
			APP.pseudoGamewebSocketInteractionController.on(PseudoGameWebSocketInteractionController.EVENT_ON_SERVER_ERROR_MESSAGE, this._onPseudoGameServerErrorMessage, this);
	...
	_onGameExternalMessageReceived(event)
	{
		...
		switch (msgType)
		{
			case GAME_MESSAGES.PENDING_OPERATION_STATUS_UPDATED:
				...
				this.emit(LobbyPendingOperationController.EVENT_ON_ROOM_PENDING_OPERATION_STATUS_UPDATED);
...
export default LobbyPendingOperationController;