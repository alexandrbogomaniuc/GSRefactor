import DOMLayout, { DOM_LAYERS } from '../../../unified/view/layout/DOMLayout';
import EventDispatcher from '../../../unified/controller/events/EventDispatcher';
import GUSLobbyCommonPanelController from '../../controller/uis/custom/commonpanel/GUSLobbyCommonPanelController';
...
/** @ignore */
class GUSLobbyLayout extends DOMLayout 
{
	...
	containsHtmlElement(aElement_html)
	{
		return this._fHtmlOverlayNode_html.contains(aElement_html);
	}
	...
	{
		let layerName = DOM_LAYERS.COMMON_PANEL + "_" + GUSLobbyCommonPanelController.LAYER_ID_MOBILE_BUTTONS;
	...
	onGameReady()
	{
		this.emit(GUSLobbyLayout.EVENT_ON_GAME_READY);
	}
	...
	resizeMobileCommonGroupButtonsLayer(aButtonsAmount)
	{
		let layerName = DOM_LAYERS.COMMON_PANEL + "_" + GUSLobbyCommonPanelController.LAYER_ID_MOBILE_BUTTONS;
		let lNode = this.getLayer(layerName);
		let lSubNode = this._getLayerNode(layerName);

		if (lNode)
		{
			let layersParams_obj_arr = GUSLobbyCommonPanelController.getCommonPanelLayers(this.isMobile);
			let commonPanelSize_obj = GUSLobbyCommonPanelController.getCommonPanelSize(this.isMobile);
			let params = layersParams_obj_arr[GUSLobbyCommonPanelController.LAYER_ID_MOBILE_BUTTONS];

			params.top = -aButtonsAmount * 60 + 3;
			params.height = aButtonsAmount * 60;
			lSubNode.height = aButtonsAmount * 60;

			Object.assign(lNode.style, {
	...
	_resizeLayers(appRect)
	{
		...
		if (node)
		{
			let commonPanelBaseHeight = GUSLobbyCommonPanelController.getCommonPanelSize(this.isMobile).height;
			let commonPanelTop = appRect.top;
			let layersParams_obj_arr = GUSLobbyCommonPanelController.getCommonPanelLayers(this.isMobile);

			let commonPanelHeight = Math.ceil(commonPanelBaseHeight*this.scale);
			commonPanelTop = Math.ceil(commonPanelTop + appRect.height + layersParams_obj_arr[GUSLobbyCommonPanelController.LAYER_ID_BASE].top*this.scale - commonPanelHeight);

			Object.assign(node.style, {
				left: appRect.left + "px",
				top: commonPanelTop + "px",
				width: appRect.width + "px",
				height: commonPanelHeight + "px"
			});
			this._resizeLayerNode(this.getLayer(DOM_LAYERS.VUE_SCREEN), { left: appRect.left, top: appRect.top, width: appRect.width, height: appRect.height - commonPanelHeight});
	...
	addLayer(name, container = this.container) 
	{
		...
		if (name === DOM_LAYERS.COMMON_PANEL)
		{
			...
			Object.assign(node.style, {
				top: layersParams_obj_arr[GUSLobbyCommonPanelController.LAYER_ID_BASE].top * this.scale+"px",
			});
			...
			for (var i = 0; i < layersParams_obj_arr.length; i++)
			{
				...
				if (i == GUSLobbyCommonPanelController.LAYER_ID_MOBILE_BUTTONS )
				{
					Object.assign(subNode.style, {
						left: `${(params.left || 0) / commonPanelSize_obj.width * 100}%`,
						top: `${((params.top + layersParams_obj_arr[GUSLobbyCommonPanelController.LAYER_ID_BASE].top) || 0) / commonPanelSize_obj.height * 100}%`,
						width: `${params.width ? params.width / commonPanelSize_obj.width * 100 : 100}%`,
						height: `${params.height ? params.height / commonPanelSize_obj.height * 100 : 100}%`,
						pointerEvents: params.pointerEvents || 'inherit'
					});
