...
class InputText extends Sprite 
{
	...
	static get EVENT_ON_CLICK()				{ return "onInputAreaClick"; }
	...
	/**
	 * @constructor
	 * @param {Object} inputData - Input field params.
	 */
	...
	/**
	 * Update entered text.
	 * @param {string} aValue_str - New text.
	 * @param {boolean} aOptApplyFilter_bl - If TRUE - symbols and length will be checked and corrected.
	 */
	setValue(aValue_str, aOptApplyFilter_bl = true)
	{
		...
		this.leftEdge = 0;
		this.rightEdge = this.hiddenValue.length;
		this.renderText();
	}
	...
	updateFont(fontName)
	{
		...
		if (this.pixiTextField)
		{
			let txtStyle = this.pixiTextField.getStyle() || {};
			txtStyle.fontFamily = fontName;
			this.pixiTextField.textFormat = txtStyle;
		}
	}
	...
	/**
	 * Update font color.
	 * @param {number} afontColor_str 
	 */
	updateFontColor(afontColor_str) 
	{
		this.fontColor = afontColor_str || 0x000000;
		if (this.pixiTextField)
		{
			let txtStyle = this.pixiTextField.getStyle() || {};
			txtStyle.fill = afontColor_str;
			this.pixiTextField.textFormat = txtStyle;
		}
	}

	/**
	 * Init field with input params.
	...
	init(inputData)
	{
		...
		this.hiddenHTMLInput.addEventListener("focus", (e) => { 
			if (!this.hasFocus)
			{
				this.focus();
			}
	...
	setParams(inputData)
	{
		...
		this.inputWidth = (inputData.width || 150) - this.padding*2 - this.borderWidth*2;
		...
		this.clipOnFocusLost = inputData.clipOnFocusLost !== undefined ? inputData.clipOnFocusLost : true;
	...
	getParams()
	{
		let lParams_obj =
		{
			...
			width: this.inputWidth + this.padding*2 + this.borderWidth*2,
			height: this.inputHeight,
			backgroundImage: this.backImgSrc,
			...
			selectAddHeight: this.selectAddHeight,
	...
	_updateBackground()
	{
		...
		if (this.backImgSrc)
		{
			...
			this.addChildAt(this.inputBackground, 0);
			this.inputBackground.anchor.set(0.5, 0.5);

			if (this.textAlign == "left")
			{
				this.inputBackground.anchor.x = 0;
			}
			else if (this.textAlign == "right")
			{
				this.inputBackground.anchor.x = 1;
			}
		}
		else
		{
			let lY_num = -this.inputHeight / 2;
			...
			this.inputBackground = new PIXI.Rectangle(lX_num, lY_num, width, this.inputHeight);
		}
	}
	...
	_updateHitArea()
	{
		...
		{
			bounds = this.inputBackground.getLocalBounds();
		}
		else
		{
	...
	_applyAlign()
	{
		...
		switch (this.textAlign){
			case "center":
			this.pixiTextField.anchor.x = 0.5;
			break;
			case "left":
			this.pixiTextField.anchor.x = 0;
			break;
			case "right":
			this.pixiTextField.anchor.x = 1;
			break;
		}

		this.pixiTextField.anchor.y = 0.5;
	}
	...
	initElements(){
		...
		this.pixiTextContainer = this.addChild(new Sprite());
		this.pixiTextField = this.pixiTextContainer.addChild(new TextField(style));
		this.pixiTextField.text = "";
	...
	set _enabled(aValue_bl)
	{
		...
		this.hiddenHTMLInput.disabled = !this._fEnabled_bl;
	}
	...
	keydown(e){
		...
		if (this.hasFocus){
			...
			if (keyCode === 13){
				APP.isMobile && this.hideKeyboard();
				this.blur();
				e.preventDefault();
				this.onsubmit(e);
			} else if (keyCode === 9){
	...
	/**
	 * Checks whether symbol is allowed or not.
	 * @param {*} char 
	 * @returns {boolean}
	 * @protected 
	 */
	...
	filter(aValue_str){
		...
		for (let i = 0; i < aValue_str.length; ++i){
			if (this.__checkNextCharacterAcceptability(aValue_str[i], filteredValue)){
	...
	/**
	 * Check acceptability of symbol.
	 * Override in child class to adjust additional filters.
	 * @protected
	 * @param {string} aValue_str - Symbol to check.
	 * @param {string} aCurrentText_str - Already applied string.
	 */
	__checkNextCharacterAcceptability(aValue_str, aCurrentText_str)
	{
		...
		if (~lDecimalPartIndex_int)
		{
			...
			if (aValue_str == ".")
				return false;
			// rule 2: up to 2 digits in decimal part
			if (aCurrentText_str.length - lDecimalPartIndex_int > 2)
				return false;
		}
		// ...additional filter for Crash Game input values
	...
	click(e){
		...
		this.emit(InputText.EVENT_ON_CLICK);
	}
	...
	renderText(){
		...
		this.pixiTextContainer.scale.x = 1;
		if (textWidth > this._textAreaWidth)
		{
			this.pixiTextContainer.scale.x = this._textAreaWidth/textWidth;
			textWidth = this._textAreaWidth;
		}
		...
		if (!this.isCursor){
			...
			let lCursorTextWidth_num = this.getTextWidth(clipedText.substring(0, this.cursorPos - this.leftEdge));
			let lCursorTextOffset_num = 0;
			if (this.textAlign == "center")
			{
				lCursorTextOffset_num = -textWidth / 2;
			}
			else if (this.textAlign == "right")
			{
				lCursorTextOffset_num = -textWidth;
			}
			
			let cursorX = lCursorTextWidth_num + lCursorTextOffset_num;
		...
		if (this.visibleValue != clipedText){
			...
			this.pixiTextField.text = this.visibleValue;
	...
	prerenderGraphics(){
		...
		let textHeight = this.pixiTextField.height + this.selectAddHeight;

		this.textSelection.cacheAsBitmap = false;
		this.textSelection.clear();

		this.textCursor.cacheAsBitmap = false;
	...
	clipText(aValue_str){
		...
		let fillPer = textWidth / this._textAreaWidth;
	...
	get _textAreaWidth()
	{
		return (this.inputWidth - (this.padding + this.borderWidth));
	}
	...
	getTextWidth(text){
		...
		return PIXI.TextMetrics.measureText(text, this.pixiTextField.style, this.pixiTextField.style.wordWrap).width * lMult_num;
	}
	...
	getClickPos(pointerX){
		...
		{
			pointerX += this.getTextWidth(text);
		}
