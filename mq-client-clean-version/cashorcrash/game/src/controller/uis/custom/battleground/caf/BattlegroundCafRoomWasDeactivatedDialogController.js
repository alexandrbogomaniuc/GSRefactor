...
class BattlegroundCafRoomWasDeactivatedDialogController extends DialogController
{
	...
	constructor(aOptInfo_usuii, parentController)
	{
		...
		console.log("room deactivated dialog controller inited")
	...
	_onServerErrorMessage(event)
	{
		...
		{
			case GameWebSocketInteractionController.ERROR_CODES.ROOM_WAS_DEACTIVATED:
					this.__activateDialog();
				break;
		}
	}
	...
	/*_onGameMessageReceived(event)
	{
		...
		switch (msgType)
		{
			...
			case GAME_MESSAGES.SERVER_ERROR_MESSAGE_RECIEVED:
				if (
						msgData.errorCode == GameWebSocketInteractionController.ERROR_CODES.ROOM_WAS_DEACTIVATED
						&& GameWebSocketInteractionController.isFatalError(msgData.errorType)
					)
				{
					this.__activateDialog();
				}
