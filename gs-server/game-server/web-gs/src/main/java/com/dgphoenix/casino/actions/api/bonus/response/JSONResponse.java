        ...
        try {
            for (Map.Entry<String, Object> entry : outParams.entrySet()) {
                if (entry.getKey().equals(CBonus.BONUS_LIST)) {
                    List<com.abs.casino.common.cache.data.bonus.BaseBonus> bonuses = (List<com.abs.casino.common.cache.data.bonus.BaseBonus>) entry.getValue();
                    bonuses.sort(Comparator.comparingLong(com.abs.casino.common.cache.data.bonus.BaseBonus::getTimeAwarded));
                    List<com.abs.casino.actions.api.bonus.response.BaseBonus> jsonBonuses = new ArrayList<>();
                    for (com.abs.casino.common.cache.data.bonus.BaseBonus bonus : bonuses) {
                        if (bonus instanceof com.abs.casino.common.cache.data.bonus.Bonus) {
                            jsonBonuses.add(new Bonus((com.abs.casino.common.cache.data.bonus.Bonus) bonus, form));
                        } else {
                            jsonBonuses.add(new FRBonus((com.abs.casino.common.cache.data.bonus.FRBonus) bonus, form));
                        }
                    }
