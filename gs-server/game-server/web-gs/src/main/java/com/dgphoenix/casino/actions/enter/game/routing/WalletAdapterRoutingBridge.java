...
/**
 * GS compatibility bridge for wallet-adapter canary shadowing.
 * Behavior is fail-open to legacy flow: any config/read/network issue disables routing/shadow calls.
 */
public final class WalletAdapterRoutingBridge {
    private static final Logger LOG = LogManager.getLogger(WalletAdapterRoutingBridge.class);
    private static final WalletAdapterRoutingBridge INSTANCE = new WalletAdapterRoutingBridge();

    private static final String RESOURCE_CLUSTER_HOSTS = "cluster-hosts.properties";
    private static final String KEY_HOST = "WALLET_ADAPTER_HOST";
    private static final String KEY_PORT = "WALLET_ADAPTER_PORT";
    private static final String KEY_ROUTE_ENABLED = "WALLET_ADAPTER_ROUTE_ENABLED";
    private static final String KEY_CANARY_BANKS = "WALLET_ADAPTER_CANARY_BANKS";

    private static final int CONNECT_TIMEOUT_MS = 400;
    private static final int READ_TIMEOUT_MS = 700;
    private static final int MAX_BODY_BYTES = 4096;

    private WalletAdapterRoutingBridge() {
    }

    public static WalletAdapterRoutingBridge getInstance() {
        return INSTANCE;
    }
    ...
    public void shadowWalletOperation(RouteDecision decision,
                                      long bankId,
                                      String sessionId,
                                      String roundId,
                                      String accountId,
                                      ...
                                      String traceId) {
        if (decision == null || !decision.routeToWalletAdapter) {
            return;
        }
        if (isBlank(operationType) || isBlank(sessionId) || isBlank(roundId) || isBlank(operationId) || amount <= 0) {
            LOG.debug("wallet-adapter shadow skipped: bankId={}, sessionId={}, roundId={}, operationId={}, type={}, amount={}",
                    bankId, sessionId, roundId, operationId, operationType, amount);
            return;
        }

        String normalizedType = operationType.trim().toLowerCase();
        if (!"reserve".equals(normalizedType) && !"settle".equals(normalizedType) && !"refund".equals(normalizedType)) {
            LOG.debug("wallet-adapter shadow skipped: unsupported operationType={}", normalizedType);
            return;
        }

        Config config = loadConfig();
        if (!config.valid) {
            LOG.debug("wallet-adapter shadow skipped: config unavailable");
            return;
        }
        ...
        String payload = "{"
                ...
                + "\"accountId\":\"" + escapeJson(isBlank(accountId) ? "unknown" : accountId) + "\"," 
                + "\"operationId\":\"" + escapeJson(operationId) + "\"," 
                + "\"currency\":\"" + escapeJson(isBlank(currency) ? "USD" : currency) + "\"," 
                + "\"amount\":" + amount + ","
                + "\"metadata\":{"
                + "\"source\":\"gs-ngs-wallet-shadow\"," 
                + "\"traceId\":\"" + escapeJson(nullToEmpty(traceId)) + "\""
                + "}"
                + "}";

        HttpResult httpResult = postJson(config.baseUrl() + "/api/v1/wallet/" + normalizedType, payload);
        if (httpResult.status >= 200 && httpResult.status < 300) {
            LOG.debug("wallet-adapter shadow {} ok: bankId={}, sessionId={}, operationId={}, status={}, body={}",
                    normalizedType, bankId, sessionId, operationId, httpResult.status, httpResult.body);
            return;
        }

        LOG.warn("wallet-adapter shadow {} failed, fallback to legacy only: bankId={}, sessionId={}, operationId={}, status={}, body={}",
                normalizedType, bankId, sessionId, operationId, httpResult.status, httpResult.body);
    }
    ...
    private Boolean fetchRemoteDecision(Config config, String bankIdValue) {
        ...
        try {
            String endpoint = config.baseUrl() + "/api/v1/wallet/routing/decision?bankId="
            ...
            if (status < 200 || status > 299) {
                LOG.warn("wallet-adapter decision call failed: status={}, body={}", status, body);
                return null;
            }
            if (body.contains("\"routeToWalletAdapter\":true")) {
                return Boolean.TRUE;
            }
            if (body.contains("\"routeToWalletAdapter\":false")) {
                return Boolean.FALSE;
            }
            LOG.warn("wallet-adapter decision response missing routeToWalletAdapter flag: {}", body);
            return null;
        } catch (Exception e) {
            LOG.warn("wallet-adapter decision unavailable: {}", e.getMessage());
            return null;
        } finally {
            if (conn != null) {
                conn.disconnect();
    ...
    public static final class RouteDecision {
        private final boolean routeToWalletAdapter;
        private final String reason;
        private final String endpoint;

        private RouteDecision(boolean routeToWalletAdapter, String reason, String endpoint) {
            this.routeToWalletAdapter = routeToWalletAdapter;
            this.reason = reason;
        ...
        public boolean isRouteToWalletAdapter() {
            return routeToWalletAdapter;
        }
        ...
        public String toString() {
            return "RouteDecision{"
                    + "routeToWalletAdapter=" + routeToWalletAdapter
                    + ", reason='" + reason + '\''
                    + ", endpoint='" + endpoint + '\''
                    + '}';
        }
