    ...
    for (String key : gamesKeySet) {
        ...
        if (split[1].equals(bankString)) {
            ...
            BaseGameInfoTemplate baseGameInfoTemplateById = BaseGameInfoTemplateCache.getInstance().getBaseGameInfoTemplateById(
                    originalId);
            response.getWriter().write("bank: " + bank + " bank name: " + BankInfoCache.getInstance().getBankInfo(bank).getExternalBankIdDescription() + " , title: " + baseGameInfoTemplateById.getTitle() + ",   originalId:" + originalId + ",  externalGameId:  " + split[0] + "</br>");
        }
    }
