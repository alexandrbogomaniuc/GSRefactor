...
/Users/alexb/Documents/Dev/Dev_new/gs-server/deploy/scripts/phase8-precision-wave3-discrepancy-export-smoke.sh
        ...
        <p class="small-note">
            Optional viewer page for exported JSON: <code>/support/phase8DiscrepancyViewer.html</code> (supports file upload, pasted JSON, embedded samples, A-vs-B compare mode, guided validation thresholds/pass-fail badges, named threshold policy profiles, compact JSON/Markdown comparison report export, compact compare-report JSON import/inspection, imported-artifact diff mode for two compact compare reports, changed-only diff filters, drag/drop import onto Imported A/B text areas, rule-status diff filters, metric-name search triage, browser-local saved triage presets, triage preset JSON export/import for cross-machine sharing, preset-bundle drag/drop import onto the preset JSON textarea, preset-bundle schema/version compatibility status badges, preset import preview counts (incoming/new/overwrite) before merge, a configurable high-overwrite confirmation guard (strict/default/relaxed/disabled/custom threshold) with browser-local guard setting persistence, a one-click reset for viewer-local presets/guard settings, and overwrite-guard settings JSON export/import plus drag/drop import with schema/version compatibility status metadata and a pre-apply import preview for cross-machine safety-profile sharing in <code>file://</code> mode). For non-UI automation, use CLI compare/export: <code>gs-server/deploy/scripts/phase8-precision-wave3-discrepancy-compare-export.sh</code> (named policy profiles with optional per-run threshold overrides).
        </p>
    </div>
