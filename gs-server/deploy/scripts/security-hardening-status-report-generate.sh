...
node - <<'NODE' "${ROOT}" "${VERIFY_REPORT}" "${REPORT}"
const fs = require('fs');
const path = require('path');
const [root, verifyFile, outFile] = process.argv.slice(2);
...
} else if (baselineDocsPass && pkgCount >= 8 && lockCount === 0) {
...
} else if (baselineDocsPass && pkgCount >= 8 && lockCount === pkgCount && unpinnedCount === 0) {
  overall = 'TESTED_SECURITY_HARDENING_COMPLETE';
  decision = 'Go (security baseline, dependency locks, and pinning checks complete)';
}
...
out.push('- Missing lockfiles and network-constrained audit execution keep runtime dependency hardening in no-go state for cutover-level approval.');
