...
run_check "help_phase8_nonprod_canary" "CLI help: Phase 8 non-prod precision canary readiness/evidence tools" \
  bash -lc "'${ROOT}/gs-server/deploy/scripts/phase8-precision-nonprod-canary-readiness-check.sh' --help | sed -n '1,100p' && '${ROOT}/gs-server/deploy/scripts/phase8-precision-nonprod-canary-evidence-pack.sh' --help | sed -n '1,100p' && '${ROOT}/gs-server/deploy/scripts/phase8-precision-close-after-canary.sh' --help | sed -n '1,140p' && '${ROOT}/gs-server/deploy/scripts/phase8-precision-nonprod-canary-run.sh' --help | sed -n '1,160p'"
...
run_check "help_phase9_abs_patch_plan" "CLI help: Phase 9 ABS patch-plan export" \
...
run_check "help_phase0_legacy_parity_status_report" "CLI help: Phase 0 legacy parity status report generator" \
...
run_check "logic_smoke_phase9_abs_patch_plan" "Executable logic smoke: Phase 9 ABS patch-plan export" \
...
run_check "logic_smoke_phase0_legacy_parity_status_report" "Executable logic smoke: Phase 0 legacy parity status report generator" \
