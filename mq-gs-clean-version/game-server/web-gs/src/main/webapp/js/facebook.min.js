...
f.daily_config)c.SLABS=f.sl,c.R_BONUS_COUNT=f.rtcn,c.R_BONUS_COST=f.rtcs,c.L_BONUS_VALUE=f.lkcs,c.T_BONUS_VALUE=f.tmcs,c.T_BONUS_COUNT=f.tmc,c.REBUY_COST=f.rbc,c.SPIN=f.asc,c.REWARD=f.trdb,swfobject.embedSWF($.fbc.getCDN()+"/flash/moneyWheel/moneyWheel.swf?_version=7","dailyBonusSWF","100%","100%","9.0.0","/flash/fb/expressInstall.swf",c,b,d),$("#dailyBonus-cont .close").unbind().click(function(){swfobject.removeSWF("dailyBonusSWF");var b=$("#dailyBonus");b.hide();!0===b.data("is_spin")&&(b.removeData("is_spin"),
...
function loadMicroPopup(a,b){var d=function(){destroyMicroPopup();var a=$("#mpay-close").attr("for");"undefined"!==typeof a&&$("#"+a).remove();show_popan_ad("cashier")},c=$("<div id='mpcoins-selector'></div>"),f=$("<div id='mpay' class='unselectable'></div>");f.append($("<div id='mpay-flash-block' class='flash-block'></div>"),$("<div id='mpay-wraper' class='popupwraper'>").append($("<div id='mpay-page' class='unselectable'>").append($("<div id='mpay-close' for='mpay'></div>").unbind("click").click(d),
c,$("<div id='adpanel'></div>").append($("<div id='mpearnpanel'>").append($("<div class='earn-wait'><div class='earn-wait-mess'></div></div>"),$("<div class='earn-trial' style='z-index:-1;height:0;width:0;'><div class='trial-pay' id='mptrial-pay'></div></div>"),$("<div class='earn-simpl'></div>").append($("<div class='earn-butt'></div>").unbind("click").click(function(){$.fbc.earn();d()}))),$("<div id='mptbcollect'></div>"),$("<div id='mpgifts'></div>").unbind("click").click(function(){sendBribes();
d()})))));b&&b.p&&$.each(b.p,function(a,b){$("<div id='mpcoinspack"+a+"' class='coinspack'></div>").append(showNumber(b.c,"b",$("<div class='b'></div>"),np,ne,!0,"b$"),showNumber(b.v,"m",$("<div class='v'></div>"),np,ne,!1,void 0,"mcoins")).data("cp",b).unbind("click").click(function(){var a=$(this).data("cp");a&&jQuery.fbc.nMakeBuyIn(a.i,a.t)}).appendTo(c)});a.append(f);$.fbc.initTrialPayWithJFunc("mptrial-pay",function(){var a=$("#mpearnpanel");a.find(".earn-wait,.earn-simpl").hide();a.find(".earn-trial").css({"z-index":"",
height:"",width:""}).show()},function(){var a=$("#mpearnpanel");a.find(".earn-wait").hide();a.find(".earn-trial").css({"z-index":-1,height:0,width:0});a.find(".earn-simpl").show()},d)}function destroyMicroPopup(){$.fbc.destroyTrialPay("mptrial-pay")}
