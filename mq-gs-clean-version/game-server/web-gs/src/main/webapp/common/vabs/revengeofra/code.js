...
// 6: Giant Scorpion
// 7: Wrapped Minion
// 8: Wrapped Shadowguard
// 9: Wrapped Spiritguard
// 10: Tahawy Warrior
// 11: Crimson Bataanta
// 12: Emerald Bataanta
// 13: Unstable Avatar
...
// 16: Swarming Locust
// 17: Cursed Brawler
// 20: Infernal Forgemaster
// 21: Boss

var enemies = [
    engine.createSprite("0.png"),
...
function createRowEvent(row) {
    ...
    if (rowData[3] != null) {
        var playerRoundId = rowData[3].split("=")[1];
...
function initData(row) {
    ...
    for (var idEnemyType = 0; idEnemyType < 22; idEnemyType++) {
        var array = [];
        for (idx = 9; idx < length; idx++) {
    ...
    // for (var i = 0; i < pagesRowData.length; i++) {
    //     console.log(pagesRowData[i]);
    // }
...
function processScreenData(rowData, text_start, text_step, screenIndex, idxBetLevel) {
    var idx;
    // console.log("screenIndex: " + screenIndex + "  pagesRowData[screenIndex]: " +
    //     pagesRowData[screenIndex] + " pagesRowData.length: " + pagesRowData.length + " idxBetLevel: " + idxBetLevel);

    if (screenIndex === 0) {
        sprite_backgroundMain.draw();
        idx = 1;
        var roomStake = toMoney(rowData[8].split("=")[1]);
        engine.drawText("Base stats", center_x, text_start - 10, true, 13, COLOR_WHITE, true);
        engine.drawText("Room Id: " + rowData[5].split("=")[1] + " stake: " + roomStake, center_x, text_start + idx * text_step, false, 12);
        idx++;
        var kk = 0;
        var res = "";
        ...
        engine.drawText("Money Wheel cnt: " + moneyWheelCompleted + " payout: " + toMoney(moneyWheelPayouts),
