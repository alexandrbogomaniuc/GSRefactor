...
function initCoinTextures()
{
	if (!coin_textures)
	{
		let lTextures_arr = AtlasSprite.getFrames(APP.library.getAsset("boss_mode/coins_explode/boss_death_coin_silver"), AtlasConfig.BossDeathCoin, "");
		coin_textures = lTextures_arr;
	}
}

class CoinsEmitterSilver extends Sprite
{
	...
	static fillPool()
	{
		...
		if (COINS_POOL.length < CoinsEmitterSilver.poolLimit)
		{
			APP.on("tick", CoinsEmitterSilver._preGenerateParticle);
		}
	}
	...
	static _preGenerateParticle()
	{
		if (COINS_POOL.length >= CoinsEmitterSilver.poolLimit)
		{
			APP.off("tick", CoinsEmitterSilver._preGenerateParticle);
			return;
		}

		COINS_POOL.push(CoinsEmitterSilver._generateParticle(true));
	}
	...
	static _generateParticle(aForceCreate_bln = false)
	{
		...
		let lLifeTime_num = CoinsEmitterSilver._getMidValue(EMITTER_SETTINGS.lifetime.min, EMITTER_SETTINGS.lifetime.max, lRnd_num);
		let lFadingTime_num = CoinsEmitterSilver._getMidValue(EMITTER_SETTINGS.fadingtime.min, EMITTER_SETTINGS.fadingtime.max, lRnd_num);
	...
	_onTick(aEvent_obj)
	{
		...
		let lIntense_num = CoinsEmitterSilver._getMidValue(this._fIntenseProfile_obj.min, this._fIntenseProfile_obj.max);
		while (this._fIntenseTimer_num < this._fEmittingTime_num)
		{
			let lParticle_sprt = this.addChild(CoinsEmitterSilver._generateParticle());
	...
	_onFinish()
	{
		...
		this.emit(CoinsEmitterSilver.EVENT_ON_EMITTING_COMPLETED, {target: this});
	}
...
export default CoinsEmitterSilver;