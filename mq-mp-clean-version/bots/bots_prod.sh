...
function start() {
        if [ -f "$PID_FILE" ]; then
            ...
            nohup $JAVA -Xms32M -Xmx512M -DBOT_SERVER_THRIFT_HOST=gs4-ats -DMY_GPER_STOP_ID=mp-bot -DBOT_SERVER_THRIFT_PORT=6400 -DFAKE_MQB_API=true \
            -DBOT_SERVER_KAFKA_HOST=kafka1.gsmp.lan -DBOT_SERVER_KAFKA_PORT=9092 -DBOT_SERVER_KAFKA_TOPIC=to_bs_topic \
            -DDOMAIN_LAUNCH_URL=http://default-beta.discreetgaming.com -DMQB_SITE_BOT_API_URL=https://ats.maxquest.com/ \
