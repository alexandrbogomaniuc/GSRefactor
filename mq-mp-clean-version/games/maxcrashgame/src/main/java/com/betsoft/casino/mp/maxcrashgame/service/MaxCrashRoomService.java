...
import com.betsoft.casino.mp.maxcrashgame.model.GameRoom;
...
public class MaxCrashRoomService extends AbstractRoomService<GameRoom, GameRoomSnapshot, Seat, IMultiNodeRoomInfo> {
    private static final Logger LOG = LogManager.getLogger(MaxCrashRoomService.class);

    public MaxCrashRoomService(ApplicationContext context, HazelcastInstance hazelcast, String loggerDir,
    ...
    protected GameRoom create(IMultiNodeRoomInfo roomInfo, IPlayerStatsService playerStatsService, IWeaponService weaponService,
        ...
        GameMap currentMap = new GameMap(EnemyRange.BASE_ENEMIES, gameMapStore.getStartMap(GameType.MAXCRASHGAME));
        return new GameRoom(context, createRoomLogger(roomInfo.getId(), GameType.MAXCRASHGAME, roomInfo.getMoneyType()),
    ...
    public GameType getType() {
        return GameType.MAXCRASHGAME;
    }

    @Override
    public synchronized void init() {
        GameConfig config = new GameConfigLoader().loadDefaultConfig();
        gameConfigProvider.registerDefaultConfig(GameType.MAXCRASHGAME.getGameId(), config);
        super.init();
    }
}
