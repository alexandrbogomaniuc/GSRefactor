...
import static com.betsoft.casino.mp.bgmissionamazon.model.math.EnemyRange.BASE_ENEMIES;

public class GameRoom extends AbstractBattlegroundGameRoom<EnemyGame, GameMap, Seat, GameRoomSnapshot, Enemy, EnemyType, ISingleNodeRoomInfo, IBattlegroundRoomPlayerInfo> {
    ...
    private final transient PriorityQueue<Long> staticEnemiesDieOrDisappearTime;
    ...
    @Override
    public int getCurrentPowerUpMultiplierBySeat(Seat seat) {
        if (seat == null) {
            return 0;
        } else {
            return seat.getAdditionalTempCounters(AbstractActionSeat.ADD_COUNTER_POWER_UP_MULT);
        }
    }
}
