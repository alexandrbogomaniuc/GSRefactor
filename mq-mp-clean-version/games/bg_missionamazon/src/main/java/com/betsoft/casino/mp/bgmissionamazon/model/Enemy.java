...
public class Enemy extends AbstractEnemy<EnemyClass, Enemy> {
    ...
    @Override
    protected void serializeInheritorFields(JsonGenerator gen, SerializerProvider serializers)
            throws IOException {
    }

    @Override
    protected void serializeAwardedPrizes(JsonGenerator gen, SerializerProvider serializers)
            throws IOException {
        serializeListField(gen, "awardedPrizes", awardedPrizes, new TypeReference<List<IEnemyPrize>>() {});
    }

    @Override
    protected void serializeEnemyClass(JsonGenerator gen, SerializerProvider serializers)
            throws IOException {
    ...
    protected void deserializeEnemyClass(JsonParser p, JsonNode node, DeserializationContext ctxt)
            throws IOException {
        ObjectMapper om = (ObjectMapper) p.getCodec();
        enemyClass = om.convertValue(node.get("enemyClass"), EnemyClass.class);
    }

    @Override
    public boolean isLocationNearEnd(long time) {
        if (EnemyRange.FLYING_ENEMIES.getEnemies().contains(enemyClass.getEnemyType()) || isPartOfSwarm())
            return false;
        else
