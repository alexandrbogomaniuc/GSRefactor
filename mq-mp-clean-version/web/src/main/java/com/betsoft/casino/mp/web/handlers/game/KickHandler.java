...
import com.betsoft.casino.mp.transport.Kick;
...
/**
 * Handler for kick players from private rooms.
 */
@Component
public class KickHandler extends AbstractRoomHandler<Kick, IGameSocketClient> {
    private static final Logger LOG = LogManager.getLogger(KickHandler.class);
    private final LobbySessionService lobbySessionService;
    private final SocketService socketService;
    public KickHandler(IMessageSerializer serializer, SingleNodeRoomInfoService singleNodeRoomInfoService,
    ...
    /**
     * Handles Kick message from private room owner. Checks if player is seater or not, kicks him and send response to owner
    ...
    @Override
    public void handle(WebSocketSession session, Kick message, IGameSocketClient clientOwner) {
        ...
        if (!clientOwner.isOwner()) {
            ...
            sendErrorMessage(clientOwner, ErrorCodes.BAD_REQUEST, "Only owner (host) can kick players", message.getRid());
            return;
        }
        ...
        try {
            ...
            if (!room.getGameState().isAllowedKick()) {
                ...
                sendErrorMessage(clientOwner, ErrorCodes.NOT_ALLOWED_KICK, "kick not allowed in this state", message.getRid());
                return;
            }
            ...
            getLog().debug("handle: updatePlayersStatusToKick for candidateNickname: {}", candidateNickname);
            this.updatePlayersStatusToKick(room, candidateNickname);

            getLog().debug("handle: sendKickPlayerStatusToCanex for candidateNickname: {}", candidateNickname);
            this.sendKickPlayerStatusToCanex(room, roomInfo, candidateNickname);
            ...
            if(accountId != null) {
                //accountId was found for candidateNickname, make a kick process over RoomInfo Listener
                getLog().debug("handle: make a kick process over RoomInfo Listener, accountId: {}", accountId);
                this.kickPLayerOverRoomInfoListener(roomInfoService, roomInfo, accountId);
            }

            IKickResponse response = room.getTOFactoryService()
                    .createKickResponse(System.currentTimeMillis(), message.getRid());
    ...
    private void sendKickPlayerStatusToCanex(IRoom room, IRoomInfo roomInfo, String candidateNickname) {

        if(room == null) {
            getLog().error("sendKickPlayerStatusToCanex: room is null for candidateNickname={}", candidateNickname);
            return;
        }

        if(!(room instanceof ICAFRoom)) {
            getLog().error("sendKickPlayerStatusToCanex: the room is not ICAFRoom {}", room);
            return;
        }

        if(roomInfo == null) {
            getLog().error("sendKickPlayerStatusToCanex: roomInfo is null for candidateNickname={}", candidateNickname);
            return;
        }

        try {

            Status tbgStatus = Status.KICKED;

            String privateRoomId = roomInfo.getPrivateRoomId();

            getLog().debug("sendKickPlayerStatusToCanex: privateRoomId:{}, TBGStatus:{}, " +
                            "candidateNickname:{}", privateRoomId, tbgStatus, candidateNickname);

            if(StringUtils.isTrimmedEmpty(privateRoomId)) {
                getLog().error("sendKickPlayerStatusToCanex: privateRoomId is empty, skip sendKickPlayerStatusToCanex");
                return;
            }
            ...
            getLog().error("sendKickPlayerStatusToCanex: Exception {}", e.getMessage(), e);
        }
    }

    private void kickPLayerOverRoomInfoListener(IRoomInfoService roomInfoService, IRoomInfo roomInfo, long accountId) {
        getLog().debug("kickPLayer: accountId: {}, roomInfo: {}", accountId, roomInfo);
        long roomId = roomInfo.getId();
        roomInfoService.lock(roomId);
        try {
            roomInfo.kickPlayer(accountId);
    ...
    private void updatePlayersStatusToKick(IRoom room, String candidateNickname) {

        if(!(room instanceof ICAFRoom)) {
            getLog().error("updatePlayersStatusToKick: the room is not ICAFRoom {}", room);
            return;
        }

        Status status = Status.KICKED;
        try {
            getLog().debug("updatePlayersStatusToKick: room.updatePlayersStatusNicknamesOnly to {} for {} ",
            ...
            getLog().debug("updatePlayersStatusToKick: response {}", response);

            if (response == null) {
                getLog().error("updatePlayersStatusToKick: response is null for {}", candidateNickname);
                return;
            }

            if (response.getPrivateRoom() == null) {
                getLog().error("updatePlayersStatusToKick: response.getPrivateRoom() is null " +
                        "for {}", candidateNickname);
                return;
            }

            if (StringUtils.isTrimmedEmpty(response.getPrivateRoom().getPrivateRoomId())) {
                getLog().error("updatePlayersStatusToKick: " +
            ...
            if (response.getPrivateRoom().getPlayers() == null || response.getPrivateRoom().getPlayers().isEmpty()) {
                getLog().error("updatePlayersStatusToKick: response.getPrivateRoom().getPlayers() " +
                        "is empty for {}", candidateNickname);
                return;
            }

        } catch (Exception e) {
            getLog().error("updatePlayersStatusToKick: Exception to " +
