CREATE OR REPLACE VIEW `${google.cloud.project.id}.${google.cloud.bigquery.dataset.name}.concurrent_players_per_games_counts` AS
SELECT
      all_times.unique_timestamp,
      rounds_sums_total.Group_Game_Name, 
      rounds_sums_total.Room_Type, 
      SUM(
        CASE
