...
test("shell theme tokens provide strict defaults", () => {
  ...
  assert.equal(theme.preloader.heroFx, DefaultShellThemeTokens.preloader.heroFx);
  assert.equal(theme.preloader.vfxIntensity, DefaultShellThemeTokens.preloader.vfxIntensity);
});
