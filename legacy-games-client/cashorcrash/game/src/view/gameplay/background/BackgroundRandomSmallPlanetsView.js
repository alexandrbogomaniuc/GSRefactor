...
class BackgroundRandomSmallPlanetsView extends BackgroundTilesetBaseClassView
{
	constructor()
	{
		super();
	...
	expandIfRequired(aWidthInPixels_num, aHeightInPixels_num)
	{
		...
		let l_btrpv = this._fSmallPlanet_btrpv;

		if (!l_btrpv)
		{
			l_btrpv = this.generateTileView(0);

			this.addChild(l_btrpv);

			this._fSmallPlanet_btrpv = l_btrpv;
		}
	...
	adjust()
	{
		...
		if (lIsTileCycleChanged_bl)
		{
			...
			this.setRandomPlanets(lCurTileCyclesAmount_int, GameplayBackgroundView.TILESET_BIG_PLANETS.currentPlanetIndex);
		}

		let l_rcdc = this._fSmallPlanet_btrpv;
	...
	{
		this._fSmallPlanet_btrpv.randomize(aOffset_num, aExcludePlanets_int_arr);
	}

	get currentPlanetIndex()
	{
		return this._fSmallPlanet_btrpv.currentPlanetIndex;
	}
}
export default BackgroundRandomSmallPlanetsView;