...
class BattlegroundPrivatePlayersListItem extends Sprite
{
	...
	update(aData_obj, reinviteCount)
	{
		this._fItemData_obj = this._validateItemData(aData_obj);
		this._fReinviteCount = reinviteCount;

		this._updateView();
	}

	isOddPosition(aValue_bull)
	{
		this._isOdd = aValue_bull;
	}
	...
	get reinviteCount(){
		...
		if(!lNickName_str || !this._fReinviteCount[lNickName_str]) return 0;
		return this._fReinviteCount[lNickName_str];
	...
	_updateView()
	{
		...
		/*
		TODO: this APP.isCAFRoomManager will not work for guest needs fixing
		*/
		let lIsRoomManager_bl = (lItemData_obj[PLAYER_PARAMS.IS_OWNER] == true);
		...
		if(this._isOdd && this._fFriendBase_sprt.visible){
			this._fFriendBase_sprt.alpha = 1;
		}else if(!this._isOdd && this._fFriendBase_sprt.visible)
		{
			this._fFriendBase_sprt.alpha = 0;
		}

		if(lItemData_obj.pendingInvite || this.reinviteCount >=3 || APP.gameController.gameplayController.roomController.info.pendingInvite == true || APP.gameController.gameplayController.roomController.info.observers.length >= APP.gameController.gameplayController.roomController.info.maxRoomPlayers ){
		...
		if(!APP.isCAFRoomManager){
			this._fKick_btn.enabled = false;
			this._fKick_btn.alpha = 0;
			this._fClock_sprt.x = 90;
			this._fPlayerReady_sprt.x = 90;
			this._fPlayerZZZSprite.x = 90;
			this._fPlayerRedIcon_sprt.x = 90;
			this._fPlayerNotReady_sprt.x = 90;
			if(lIsRoomManager_bl)
			{
				this._fCrown_sprt.visible = true;
				this._fCrown_sprt.alpha = 1;
			}else{
				this._fCrown_sprt.visible = false;
				this._fCrown_sprt.alpha = 0;
			}
		}
	...
	_onKickBtnClicked()
	{
		this._fItemData_obj.pendingInvite = false;
	...
	{
		let lItemData_obj = this._fItemData_obj;		
		if(lItemData_obj.pendingInvite){
			return;
		}
		lItemData_obj.pendingInvite = true;
		APP.gameController.gameplayController.roomController.info.pendingInvite = true;

		if(lItemData_obj[PLAYER_PARAMS.STATUS] == "UNINVITED_FRIEND")
		{
			this.emit(BattlegroundPrivatePlayersListItem.EVENT_ON_INVITE_CLICKED);
		}else{
			this.emit(BattlegroundPrivatePlayersListItem.EVENT_ON_CANCEL_KICK_CLICKED);
		}
		let lNickName_str = lItemData_obj[PLAYER_PARAMS.NICK]
		this._fReinviteCount[lNickName_str]++;
