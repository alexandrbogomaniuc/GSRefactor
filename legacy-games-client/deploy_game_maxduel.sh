...
# Change to the /home/bitbucket/$dir_name directory
cd /home/bitbucket/"$dir_name" || { echo "Failed to change directory to $dir_name"; exit 1; }
...
for sub_dir in game lobby; do
    if [ -d "/www/html/repo/html5pc/actiongames/$dir_name/$sub_dir/" ]; then
        ...
        sudo rm -vrf /www/html/repo/html5pc/actiongames/"$dir_name"/"$sub_dir"/* || { echo "Failed to remove $sub_dir files"; exit 1; }
    else
