...
<script type="text/javascript">
	...
	import {ENEMY_TYPES, INDICATORS_CONSTANT_VALUES, BATTLEGROUND_STAKE_MULTIPLIER } from '../../../../../shared/src/CommonConstants';
	import WeaponSlider from './WeaponSliderBattleground.vue';
		...
		methods: {
			...
			_updateWeaponSliderPositionIfRequired(aPageName_str)
			{
					...
					)
				{
					...
					let lTargetPositionDescriptor_html = lRegularPositionDescriptor_html;
			...
			{
				if(aCurrentWeaponId_int == -1 & (aCurrentBetLevel_int === 1 || aCurrentBetLevel_int === 2))
				return;
			...
			_getCurrentStake()
			{
				this._fCurrentStake_int = BATTLEGROUND_STAKE_MULTIPLIER;

				return this._fCurrentStake_int;
			},
			...
			_insertPayout(aTargetSpan_htmlel, aEnemyId_int, aWeaponId_int, aCurrentStake_int, aBetLevel_int)
			{
				...
				{
					if(enemy.idEnemy === parseInt(aEnemyId_int))
					{
				...
				if(lPayout_obj.minPayout !== lPayout_obj.maxPayout)
				{
					try
					{
						let lMin_num = Math.floor(aCurrentStake_int * lPayout_obj.minPayout * aBetLevel_int * PAYOUT_MULTIPLAER) / BATTLEGROUND_STAKE_MULTIPLIER;
						let lMax_num = Math.floor(aCurrentStake_int * lPayout_obj.maxPayout * aBetLevel_int * PAYOUT_MULTIPLAER) / BATTLEGROUND_STAKE_MULTIPLIER;
						lResultString_str = lMin_num + " - " + lMax_num;
					}
					catch(e)
					{
				...
				else
				{
					...
					lResultString_str = lMin_num / BATTLEGROUND_STAKE_MULTIPLIER;
				}
				...
				this._updateGemPayoutIndicator(aBetLevel_int);
			},
			...
			instantiateSlots() 
			{
				...
				this.insertValue("#MAX_SHOTS_PER_POWERUP", INDICATORS_CONSTANT_VALUES.MAX_SHOTS_PER_POWERUP);
			...
			updateContent(page)
			{
				...
				if(
					...
					this._getPageId(page) == "legendary_enemies"
					)
				{
				...
				if(this._getPageId(page) == "special_weapons")
				{
				...
				this._replaceAll(
					"##MAX_SHOTS_PER_POWERUP##",
					INDICATORS_CONSTANT_VALUES.MAX_SHOTS_PER_POWERUP
					);

				//SW TARGETS...
				let lTargetsCounts_int_arr = INDICATORS_CONSTANT_VALUES.SPECIAL_WEAPONS_MAX_TARGETS_COUNTS_BTG;
				for( let i = 0; i < lTargetsCounts_int_arr.length; i++ )
				{
					this._replaceAll(
						"##SPECIAL_WEAPONS_MAX_TARGETS_COUNTS_" + i + "##",
						lTargetsCounts_int_arr[i]
						);
				}
				//...SW TARGETS
				...
				this._replaceAll(
					"##SLOTS_SPINS_COUNT##",
					INDICATORS_CONSTANT_VALUES.SLOTS_SPINS_COUNT
					);
				//...SLOTS SPINS COUNT
				//...SUBSTITUTE TEXT PLACEHOLDERS WITH VALUES
			...
			_updateGemPayoutIndicator(aBetLevel_int)
			{
 				//GEM PAID...
				if(this.gemPayouts && this.gemPayouts.length > 0)
				{
					for(var i = 0; i < this.gemPayouts.length; i++)
					{
						let lSpanId_str = "#GEM_PAID_" + i;
						var lFormatPayouts_str = APP.currencyInfo.i_formatNumber(this.gemPayouts[i][0] * this._getCurrentStake() * aBetLevel_int, true);
						this.insertValue(lSpanId_str, lFormatPayouts_str);
						
						let lSpan_htmlel = this.$refs.mainContainer.querySelectorAll('span[class=gem_paid_' + i + ']');
						let lPayoutMaxTextWidth_num = 206;
						let lOriginWidth_num = this.getTextWidth(lFormatPayouts_str, "15px fnt_nm_barlow_bold");

						if(lOriginWidth_num > lPayoutMaxTextWidth_num)
						{
							let lScale_num = (lPayoutMaxTextWidth_num * 100) / lOriginWidth_num / 100;

							Object.assign(lSpan_htmlel[0].style, {
										"transform": "scaleX(" + lScale_num + ")",
									});
						}
					}
