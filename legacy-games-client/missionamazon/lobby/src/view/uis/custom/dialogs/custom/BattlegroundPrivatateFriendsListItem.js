...
class BattlegroundPrivatateFriendsListItem extends Sprite {
	...
	_updateView() {
		...
		if(lItemData_obj.pendingInvite || this._fPlayerInfo_pi.roomObservers.length >= 6 || this._fPlayerInfo_pi.pendingInvite == true)
		{
			this._invite_btn.enabled = false;
		}else{
			this._invite_btn.enabled = true;
		}
	...
	_onInviteButtonClicked(event) {
		...
		this._fPlayerInfo_pi.pendingInvite = true;
