...
import RicochetLasersInfo from '../../../../model/uis/weapons/ricochet_laser/RicochetLasersInfo';
import RicochetLasersView from '../../../../view/uis/weapons/ricochet_laser/RicochetLasersView';
...
class RicochetLasersController extends SimpleUIController {
	...
	__initViewLevel()
	{
		...
		this.view.on(RicochetLasersView.EVENT_ON_BEAM_BASIC_ANIMATION_COMPLETED, this._onSomeBeamBasicAnimationCompleted, this);
	}
	...
	_onTimeToShowFire(aEvent_obj)
	{
		...
		if (data.usedSpecialWeapon === WEAPONS.RICOCHET || data.usedSpecialWeapon === WEAPONS.RICOCHET_POWER_UP)
		{
	...
	{
		let lStartPos_pt = this._gameScreen.gameField.getSeat(data.seatId, true).muzzleTipGlobalPoint;
	...
	_onSomeBeamBasicAnimationCompleted(aEvent_obj)
	{
		...
		if (ShotResultsUtil.isMasterShot(lShotData_obj) && !this.view.hasNotReadyMasterBeams)
		{
			this.emit(RicochetLasersController.EVENT_ON_MAIN_SPOT_BEAM_BASIC_ANIMATION_COMPLETED);
		}
	}
...
export default RicochetLasersController;