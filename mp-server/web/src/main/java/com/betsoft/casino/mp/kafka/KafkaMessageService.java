...
public class KafkaMessageService {
    ...
    void init() {
        ...
        properties.setProperty(ConsumerConfig.GROUP_ID_CONFIG, "cg-mp-reply-spk");
        ...
        initReplyConsumer(properties);
    }

    private void initReplyConsumer(Properties properties) {
        List<String> topics = Arrays.asList(
                kafkaProperties.getKafkaReplyFromGsTopic(),
                kafkaProperties.getKafkaReplyInServiceTopic(),
                kafkaProperties.getKafkaReplyFromBotServiceTopic());
                                    ...
                                    try {
                                        ...
                                        awaitingCompletableFutures.computeIfPresent(record.key(), new BiFunction<String, CompletableFuture<KafkaResponse>, CompletableFuture<KafkaResponse>>() {
                                            ...
                                            public CompletableFuture<KafkaResponse> apply(String messageId, CompletableFuture<KafkaResponse> completableFuture) {
                                                LOGGER.debug("Needed response is received " + messageId + " :: " + response.getClass());
        ...
        kafkaReplyConsumerThread.setName("kafka-consumption-" + topics);
    ...
    public void asyncRequestToRandomGs(KafkaRequest request) {
        String kafkaTopic = kafkaProperties.getKafkaSendToRandomGsTopic();
        sendToKafka(request, kafkaTopic, null);
    }

    public void asyncRequestToAllMP(KafkaRequest request) {
        String kafkaSendToMpTopic = kafkaProperties.getKafkaSendInServiceAllTopic();
        sendToKafka(request, kafkaSendToMpTopic, null);
    }

    public void asyncRequestToSpecificMP(KafkaRequest request, long receiverServerId) {
        String kafkaSendToSpecificMpTopic = kafkaProperties.getKafkaSendInServiceSpecificTopic();
        sendToKafka(request, kafkaSendToSpecificMpTopic, KafkaConfiguration.fromServerId(receiverServerId));
    }

    public Mono<KafkaResponse> syncRequestToRandomGs(KafkaRequest request) {
        String kafkaTopic = kafkaProperties.getKafkaSendToRandomGsTopic();
        final String messageId = sendToKafka(request, kafkaTopic, null);
        return getResponse(messageId);
    }

    public Mono<KafkaResponse> syncRequestToSpecificGs(KafkaRequest request, long receiverServerId) {
        String kafkaTopic = kafkaProperties.getKafkaSendToSpecificGsTopic();
        final String messageId = sendToKafka(request, kafkaTopic,  "gs-" + receiverServerId);
        return getResponse(messageId);
    }

    public Mono<KafkaResponse> syncRequestToSpecificMP(KafkaRequest request, long receiverServerId) {
    ...
    public Mono<KafkaResponse> syncRequestToBotService(KafkaRequest request) {
        String kafkaTopic = kafkaProperties.getKafkaSendToBotServiceTopic();
        final String messageId = sendToKafka(request, kafkaTopic, null);
        return getResponse(messageId);
    }

    private Mono<KafkaResponse> getResponse(final String messageId) {
    ...
    private static KafkaProducer<String, JsonNode> createProducer(String kafkaHosts) {
