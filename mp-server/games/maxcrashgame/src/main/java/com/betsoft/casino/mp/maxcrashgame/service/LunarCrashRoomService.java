...
import com.betsoft.casino.mp.maxcrashgame.model.LunarCrashGameRoom;
...
public class LunarCrashRoomService extends AbstractRoomService<LunarCrashGameRoom, GameRoomSnapshot, Seat, IMultiNodeRoomInfo> {
    private static final Logger LOG = LogManager.getLogger(LunarCrashRoomService.class);

    public LunarCrashRoomService(ApplicationContext context, HazelcastInstance hazelcast, String loggerDir,
    ...
    @Override
    protected LunarCrashGameRoom create(IMultiNodeRoomInfo roomInfo, IPlayerStatsService playerStatsService, IWeaponService weaponService,
        ...
        GameMap currentMap = new GameMap(EnemyRange.BASE_ENEMIES, gameMapStore.getStartMap(GameType.LUNARCASH));
        return new LunarCrashGameRoom(context, createRoomLogger(roomInfo.getId(), GameType.LUNARCASH, roomInfo.getMoneyType()),
    ...
    @Override
    protected LunarCrashGameRoom repair(IMultiNodeRoomInfo roomInfo, IPlayerStatsService playerStatsService, IWeaponService weaponService,
                                            IPlayerQuestsService playerQuestsService,
    ...
    public GameType getType() {
        return GameType.LUNARCASH;
    }

    @Override
    public synchronized void init() {
        GameConfig config = new GameConfigLoader().loadDefaultConfig();
        gameConfigProvider.registerDefaultConfig(GameType.LUNARCASH.getGameId(), config);
        super.init();
    }
}
