...
public class EnemyGame extends AbstractEnemyGame<Enemy, Seat, EnemyType> {
    ...
    public EnemyGame(Logger logger, IGameConfigService<? extends IGameConfigService<?>> gameConfigService,
        ...
        super(logger, GameType.BG_MAXCRASHGAME, gameConfigService);
    ...
    private MaxBlastChampionsTestStand getTestStand(long accountId, String sessionId) {
        if (sessionId != null) {
            ...
            if (featureBySid != null) {
                return new MaxBlastChampionsTestStand(featureBySid);
            }
        }
        return new MaxBlastChampionsTestStand();
    }
    ...
    public GameConfig getConfig(Seat seat) {
        return (GameConfig) gameConfigProvider
                .getConfig(GameType.BG_MAXCRASHGAME.getGameId(), seat.getPlayerInfo().getRoomId());
    }

    public GameConfig getConfig(long roomId) {
        return (GameConfig) gameConfigProvider.getConfig(GameType.BG_MAXCRASHGAME.getGameId(), roomId);
    }
