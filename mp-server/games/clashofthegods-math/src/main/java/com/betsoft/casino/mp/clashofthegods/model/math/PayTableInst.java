...
public class PayTableInst {
    ...
    private static Paytable createPayTable() {
        ...
        Arrays.stream(values()).forEach(specialWeaponType -> {
            if (specialWeaponType.getAvailableGameIds().contains((int) GameType.CLASH_OF_THE_GODS.getGameId())) {
        ...
        Paytable paytable = new Paytable(enemyPayouts, energyBoss, lootboxPrizes, weaponPaidMultiplier,
                MathData.getMoneyWheelPayouts(), healthByLevels, getEnemyPayoutsByWeapons(), MathData.getPossibleBetLevels());
    ...
    public static Map<Integer, Map<Integer, Prize>> getEnemyPayoutsByWeapons() {
        ...
        for (EnemyType enemyType : EnemyType.values()) {
            ...
            IEnemyData enemyData = MathData.getEnemyData(enemyId);
            ...
            for (SpecialWeaponType weaponType : SpecialWeaponType.values()) {
                if (weaponType.getAvailableGameIds().contains((int) GameType.CLASH_OF_THE_GODS.getGameId())) weapons.add(weaponType.getId());
            }
            for (Integer weapon : weapons) {
                ...
                for (int i = 0; i < levels.length; i++) {
                    ...
                    WeaponData weaponData = enemyData.getWeaponDataMap(weapon, i, false);
