...
import com.betsoft.casino.mp.revengeofra.model.*;
...
public class TestModel {
    ...
    public static GameType gameType = GameType.REVENGE_OF_RA;
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < 100; i++) {
            ...
            testModel.doTestRound(debug,
                    ...
                    needSaveToFile, idEnemiesForShooting, numberOfEnemies, numberOfBuyWeaponsOnStartRound,
    ...
    public void doTestRound(boolean debug, Set<EnemyType> enemiesForSpecialWeapons, int cntPlayers,
                            ...
                            int cntRealShotsRequired, int betLevel, boolean needUsePaidWeapons, int paidWeaponId, String fileStatsName
    ) throws Exception {
        ...
        while (!needFinish) {
            ...
            while (currentRoom.getGameState().getRoomState().equals(RoomState.PLAY)) {
                ...
                if (seat.getAmmoAmount() < betLevel * 50 || seat.getAmmoAmountTotalInRound() == 0) {
                    // printLog("buy shots");
                ...
                boolean needPaidMineShot = isPaidSpecialShot && paidWeaponId == SpecialWeaponType.Landmines.getId();
                try {
                    if (allowSpecialWeaponsShoots) {
                        if ((notUsedMineShoots > 0 && seat.getSeatMines().isEmpty()) || needPaidMineShot) {
                            ...
                            try {
                                gameState.placeMineToMap(seat, new StubMineCoordinates(System.currentTimeMillis(), cnt, x, y,
                                        needPaidMineShot));
                                gameState.calculateMinesOnMap(true);
                            } catch (Exception e) {
                            }
                            //Thread.sleep(2);
                            continue;
                ...
                try {
                    boolean playerHasMines = !seat.getSeatMines().isEmpty();
                    if (needUsePaidWeapons && playerHasMines)
                        continue;
...
//                    if(isPaidSpecialShot){
//                        System.out.println("paid shot: " + shot);
//                    }
    ...
    private static String getDebugFileName(boolean allowSpecialWeaponsShoots, String idEnemiesForShooting, int coinInCents, int cntRealShotsRequired, int betLevel, boolean needUsePaidWeapons, int paidWeaponId) {
...
//                        System.out.println(totalPlayerRoundsInfo.getRTPStatData(829));
                    } catch (Exception e) {
