package com.betsoft.casino.mp.piratespov;
...
public class TestMathModel {
    GameType gameType = GameType.PIRATES_POV;
    GameMap currentMap;
    ...
    Map<Integer, AtomicInteger> weaponsQuests = new HashMap<>();
    PrintWriter pwh;
    ...
    public static void main(String[] args) throws CommonException, FileNotFoundException, UnsupportedEncodingException {
        ...
        int idEnemiesForShooting = EnemyType.ENEMY_4.getId();
        int coinInCents = 20;
        ...
        testMathModel.doTest(needUsePaidWeapons, allowSpecialWeaponsShoots, cntRealShotsRequired,
                enemyType, coinInCents, needSaveToFile, paidWeaponId, betLevel, levelId);
    }
    ...
    void doTest(boolean needPaidShots, boolean allowSpecialWeaponsShoots, int totalNumberShots, EnemyType enemyType,
                int coinInCents, boolean needSaveToFile, int paidWeaponId, int betLevel, int levelId)
            throws CommonException, FileNotFoundException, UnsupportedEncodingException {

        long start = System.currentTimeMillis();
        double tBet = 0;
        double baseWin = 0;
        double[] questWin = {0, 0, 0};
        double[] questWeaponsWins = {0, 0, 0};
        int cntQuestCompleted = 0;
        int cntKeys = 0;
        int cntTotalKeys = 0;
        int[] cntChests = {0, 0, 0};
        int cntTotalQuests = 0;
        double killAward = 0;

        AtomicReference<Double> weaponCompensateInternal = new AtomicReference<>((double) 0);
        ...
        Enemy enemy = createNewEnemy(enemyType, levelId);
        ...
        for (int idx = 0; idx < totalNumberShots; idx++) {
            ...
            for (int cnt = 0; cnt < size; cnt++) {
                ...
                ShootResult shootResult = ShotCalculator.shootBaseEnemy(seat, weapon, stake, enemy, false, false,
                        false, 1, factoryService, null, null);
                ...
                if (newKeyOnShot) {
                    ...
                    if (cntKeys >= 3) {
                        ...
                        int idxTreasure = randomNumberKeyFromMapWithNorm.ordinal();
                        double questWinChest = stake.getWithMultiplier(MathQuestData.questsCacheWins.get(idxTreasure + 1))
                                .toDoubleCents() / 100;
                        questWin[idxTreasure] += questWinChest;
                        cntChests[idxTreasure]++;
                        cntKeys = 0;
                    }
                }
                ...
                if (shootResult.isDestroyed()) {
                    enemy = createNewEnemy(enemyType, levelId);
                    cntEnemyKilled++;
                }
            }
        }
        ...
        seat.getWeaponSurplus().forEach(iWeaponSurplus -> weaponCompensateInternal
                .updateAndGet(v -> (double) (v + iWeaponSurplus.getWinBonus() / 100.)));
        ...
        double baseWins = baseWin + killAward + weaponCompensateDrop + weaponCompensateKillAward;
        ...
        double totalQuestWins = 0;
        for (int idx = 0; idx < 3; idx++) {
            printLog("Treasure: " + Treasure.values()[idx]);
            printLog("Quest cache Win: " + questWin[idx] + ", RTP: " + questWin[idx] / totalBet + ", cnt: " + cntChests[idx]);
            printLog("Quest Weapon Win: " + questWeaponsWins[idx] + ", RTP: " + questWeaponsWins[idx] / totalBet + ", cnt: " + cntChests[idx]);
            totalQuestWins += questWin[idx] + questWeaponsWins[idx];
        }
        printLog("Quest cntQuestCompleted: " + cntQuestCompleted + ", %: " + totalNumberShots / cntQuestCompleted);
        printLog("Quest key freq: " + (totalNumberShots / cntQuestCompleted / 3));
        printLog("Quest total  Win: " + totalQuestWins + ", RTP: " + totalQuestWins / totalBet + ", cntQuestCompleted: " + cntQuestCompleted);


        printLog("weaponCompensateInternal: " + weaponCompensateInternal.get());
        printLog("Total RTP: " + (baseWins + weaponWins + weaponPaidWins + totalQuestWins + weaponCompensateInternal.get()) / totalBet);
    ...
    double getCompensationForWeaponWithoutBetLevel(int weaponId, int shots) {
        Double rtpForWeapon = MathData.getFullRtpForWeapon(weaponId) / 100;
        Money newCompensation = Money.ZERO;
        double multiplier = new BigDecimal(shots, MathContext.DECIMAL32)
                .multiply(new BigDecimal(rtpForWeapon, MathContext.DECIMAL32))
    ...
    Pair<Integer, Integer> getRandomWeapon(boolean allowSpecialWeaponsShoots) {
        if (allowSpecialWeaponsShoots) {
            ...
            for (Map.Entry<Integer, AtomicInteger> weapon : weaponsQuests.entrySet()) {
                if (weapon.getValue().get() > 0) {
                    weapon.getValue().decrementAndGet();
                    weaponsQuests.put(weapon.getKey(), weapon.getValue());
                    return new Pair<>(weapon.getKey(), WeaponSource.QUEST.ordinal());
                }
            }
    ...
    Enemy createNewEnemy(EnemyType enemyType, int levelId) {
