...
public class GameMap extends AbstractGameMap<Enemy, GameMapShape, EnemyRange, EnemyType, GameMap> {
    ...
    @Override
    protected void serializeEnemies(JsonGenerator gen, SerializerProvider serializers)
            throws IOException {
    ...
    protected void deserializeAdditionalFields(JsonParser p, JsonNode node, DeserializationContext ctxt) {
        ...
        minBossReturnTime = node.get("minBossReturnTime").longValue();
        bossHP = node.get("bossHP").intValue();
        fragments = node.get("fragments").intValue();
    }
    ...
    public Enemy spawnBoss(long time, int duration) {
        ...
        boss.setEnergy(bossHP > 0 ? bossHP : boss.getFullEnergy());
