...
import com.betsoft.casino.mp.dragonstone.model.*;
...
public class TestModel {
    ...
    public static GameType gameType = GameType.DRAGONSTONE;
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < 1; i++) {
            ...
            String idEnemiesForShooting = "";
            int numberOfEnemies = 3;
            int coinInCents = 2;
            int cntRealShotsRequired = 500000;
            int betLevel = 3;
            boolean needUsePaidWeapons = true;
            ...
            for (String arg : args) {
                ...
                switch (params[0]) {
                    ...
                    case "allowSpecialWeaponsShoots":
                        allowSpecialWeaponsShoots = params[1].equals("true");
                        break;
                    case "needUsePaidWeapons":
                        needUsePaidWeapons = params[1].equals("true");
                        break;
                    case "betLevel":
                        betLevel = Integer.parseInt(params[1]);
                        break;
                    case "shootFromSpecialWeaponsOnly":
                        shootFromSpecialWeaponsOnly = params[1].equals("true");
                        break;
                    ...
                    case "cntRealShotsRequired":
                        ...
                        break;
                    case "paidWeaponId":
                        paidWeaponId = Integer.parseInt(params[1]);
                        break;
                    default:
                        break;
                }
    ...
    public void doTestRound(boolean debug, Set<EnemyType> enemiesForSpecialWeapons, int cntPlayers,
                            ...
                            int coinInCents,
                            int cntRealShotsRequired, int betLevel, boolean needUsePaidWeapons, int paidWeaponId) throws Exception {
        ...
        String fileStatsName = getDebugFileName(allowSpecialWeaponsShoots, idEnemiesForShooting, coinInCents, cntRealShotsRequired, betLevel, needUsePaidWeapons, paidWeaponId, gameName);
        ...
        gameMapStore.init();
        currentMap = new GameMap(EnemyRange.BASE_ENEMIES, gameMapStore.getMap(601));
        GameConfig config = new GameConfigLoader().loadDefaultConfig();
        printLog("mode: RTP: " + config.getGameRTP());
        GameRoom currentRoom = getCurrentRoom(config, roomInfo, seats, currentMap);
        ...
        while (!needFinish) {
            ...
            while (currentRoom.getGameState().getRoomState().equals(RoomState.PLAY)) {
                ...
                boolean isPaidShotIsPossible = !seat.isAnyWeaponShotAvailable() || (needUsePaidWeapons && !allowSpecialWeaponsShoots);
                boolean isPaidSpecialShot = isPaidShotIsPossible && needUsePaidWeapons;
                try {
                    if (isPaidSpecialShot) {
    ...
    private static String getDebugFileName(boolean allowSpecialWeaponsShoots, String idEnemiesForShooting, int coinInCents, int cntRealShotsRequired, int betLevel, boolean needUsePaidWeapons, int paidWeaponId, String gameName) {
        String fileStatsName = gameName + "_"
        ...
        for (Integer enemyTypeId : enemiesForShooting) {
            ...
            if (gameState.isAllowSpawn()) {
                ...
                while (cntLiveEnemies++ < numberOfEnemies) {
                    Enemy enemy = map.createEnemy(EnemyType.getById(enemyTypeId), 1, getRandomTrajectory(),
                            4, null, -1);
    ...
    private GameRoom getCurrentRoom(GameConfig config, ISingleNodeRoomInfo roomInfo, List<Seat> seats, GameMap currentMap) {
...
//                        System.out.println(totalPlayerRoundsInfo.getRTPStatData(838));
                ...
                currentMap.resetDragonStoneFragments();
        ...
        return new GameRoom(ctx, new EmptyLogger(), roomInfo, currentMap,
                ...
                null, null, null, null,
                new StubGameConfigProvider(config), null);
    }
