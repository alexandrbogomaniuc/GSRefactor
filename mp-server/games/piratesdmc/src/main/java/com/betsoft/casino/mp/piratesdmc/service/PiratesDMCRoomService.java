package com.betsoft.casino.mp.piratesdmc.service;
...
public class PiratesDMCRoomService extends AbstractRoomService<GameRoom, GameRoomSnapshot, Seat, ISingleNodeRoomInfo> {
    private static final Logger LOG = LogManager.getLogger(PiratesDMCRoomService.class);

    public PiratesDMCRoomService(ApplicationContext context, HazelcastInstance hazelcast, String loggerDir,
    ...
    protected GameRoom create(ISingleNodeRoomInfo roomInfo, IPlayerStatsService playerStatsService, IWeaponService weaponService,
        ...
        GameMap currentMap = new GameMap(EnemyRange.BaseEnemies, gameMapStore.getStartMap(GameType.DMC_PIRATES));
        return new GameRoom(context, createRoomLogger(roomInfo.getId(), GameType.DMC_PIRATES, roomInfo.getMoneyType()),
