...
import static com.betsoft.casino.mp.piratesdmc.model.EnemyRange.*;
...
public class PlayGameState extends AbstractActionPlayGameState<GameRoom, Seat, GameMap, PlayGameState> {
    ...
    protected void updateWithLock() throws CommonException {
        ...
        if (!isManualGenerationEnemies() && !needWaitingWhenEnemiesLeave  &&
                !notAllowLeviathan.get() && subround.equals(PlaySubround.BASE) && !getRemainingBosses().isEmpty()
        ...
        if (allowSpawn && !isManualGenerationEnemies()) {
            boolean notAllowTrajectoriesViaHatch = isLeviathanLive() ||
                    (!getRemainingBosses().isEmpty() && getCurrentMapId() == 802 && notAllowLeviathan.get());
    ...
    private boolean isLeviathanLive() {
        ...
        if (bossId != -1 && getCurrentMapId() == 802) {
        ...
        switch (subround) {
            case BASE:
                if (mainBossIsAvailable && !getRemainingBosses().isEmpty()) {
                    ...
                    if (bossSkinId == LEVIATHAN_SKIN_ID && getCurrentMapId() != 802) {
