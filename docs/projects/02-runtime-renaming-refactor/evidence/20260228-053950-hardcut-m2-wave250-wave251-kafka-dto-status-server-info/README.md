...
## Scope
- Batch A declarations (`target-batchA.txt`): common-gs kafka status/server-info/request DTO package migrations.
- Bounded rewires: direct import updates in corresponding handlers and `RemoteCallHelper` only.
...
## Validation
- Fast gate batchA rerun1: `STEP06 FAIL` (`rc=1`) due moved DTOs losing same-package visibility to `BGOnlinePlayerDto` / `PromoNotificationType`.
