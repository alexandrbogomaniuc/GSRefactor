...
STAB_LOG="$OUT/stabilization-prewarm-rerun2.log"
: > "$STAB_LOG"
{
...
# 2) fast gate batchA rerun2
run_fast_gate "A" "$OUT/fast-gate-runner-batchA-rerun2.log" "$OUT/fast-gate-status-batchA-rerun2.txt"
# 3) fast gate batchB rerun2
run_fast_gate "B" "$OUT/fast-gate-runner-batchB-rerun2.log" "$OUT/fast-gate-status-batchB-rerun2.txt"

# 4) Full matrix rerun2
...
# summary artifact
{
  ...
  echo "fast_gate_batchA_status=$(cut -d= -f2 "$OUT/fast-gate-status-batchA-rerun2.txt" | sed -n '1p')"
  echo "fast_gate_batchA_first_failed_step=$(cut -d= -f2 "$OUT/fast-gate-status-batchA-rerun2.txt" | sed -n '2p')"
  echo "fast_gate_batchB_status=$(cut -d= -f2 "$OUT/fast-gate-status-batchB-rerun2.txt" | sed -n '1p')"
  echo "fast_gate_batchB_first_failed_step=$(cut -d= -f2 "$OUT/fast-gate-status-batchB-rerun2.txt" | sed -n '2p')"
  echo "prewarm_status=$(cut -d= -f2 "$OUT/prewarm-status-rerun2.txt" | sed -n '1p')"
  echo "prewarm_first_failed_step=$(cut -d= -f2 "$OUT/prewarm-status-rerun2.txt" | sed -n '2p')"
  echo "validation_status=$(cut -d= -f2 "$OUT/validation-status-rerun2.txt" | sed -n '1p')"
  echo "validation_first_failed_step=$(cut -d= -f2 "$OUT/validation-status-rerun2.txt" | sed -n '2p')"
} > "$OUT/rerun2-summary.txt"
