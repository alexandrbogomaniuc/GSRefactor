...
STAB_LOG="$OUT/stabilization-prewarm-rerun4.log"
: > "$STAB_LOG"
{
...
# 2) fast gate batchA rerun4
run_fast_gate "A" "$OUT/fast-gate-runner-batchA-rerun4.log" "$OUT/fast-gate-status-batchA-rerun4.txt"
# 3) fast gate batchB rerun4
run_fast_gate "B" "$OUT/fast-gate-runner-batchB-rerun4.log" "$OUT/fast-gate-status-batchB-rerun4.txt"

# 4) Full matrix rerun4
...
# summary artifact
{
  ...
  echo "fast_gate_batchA_status=$(cut -d= -f2 "$OUT/fast-gate-status-batchA-rerun4.txt" | sed -n '1p')"
  echo "fast_gate_batchA_first_failed_step=$(cut -d= -f2 "$OUT/fast-gate-status-batchA-rerun4.txt" | sed -n '2p')"
  echo "fast_gate_batchB_status=$(cut -d= -f2 "$OUT/fast-gate-status-batchB-rerun4.txt" | sed -n '1p')"
  echo "fast_gate_batchB_first_failed_step=$(cut -d= -f2 "$OUT/fast-gate-status-batchB-rerun4.txt" | sed -n '2p')"
  echo "prewarm_status=$(cut -d= -f2 "$OUT/prewarm-status-rerun4.txt" | sed -n '1p')"
  echo "prewarm_first_failed_step=$(cut -d= -f2 "$OUT/prewarm-status-rerun4.txt" | sed -n '2p')"
  echo "validation_status=$(cut -d= -f2 "$OUT/validation-status-rerun4.txt" | sed -n '1p')"
  echo "validation_first_failed_step=$(cut -d= -f2 "$OUT/validation-status-rerun4.txt" | sed -n '2p')"
} > "$OUT/rerun4-summary.txt"
