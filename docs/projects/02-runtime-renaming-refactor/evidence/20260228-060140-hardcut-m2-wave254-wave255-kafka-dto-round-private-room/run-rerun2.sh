...
OUT="/Users/alexb/Documents/Dev/Dev_new/docs/projects/02-runtime-renaming-refactor/evidence/20260228-060140-hardcut-m2-wave254-wave255-kafka-dto-round-private-room"
  ...
  for s in STEP01 STEP02 STEP03 STEP04 STEP05 STEP06 STEP07 STEP08 STEP09; do
    ...
    if ! run_step "$s" "$d" "$c" "$VAL_RUNNER" "$VAL_RUNNER"; then
      ...
      if [ "$s" = "STEP09" ]; then
        ...
        : > "$RETRY_LOG"
        run_step "STEP09-retry1" "." "node gs-server/deploy/scripts/refactor-onboard.mjs smoke" "$RETRY_LOG" "$VAL_RUNNER"
        step09_retry_rc=$?
      fi
      break
    fi
