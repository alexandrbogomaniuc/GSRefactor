...
run_fast_gate "$OUT/fast-gate-runner-batchA-rerun5.log" "$OUT/fast-gate-status-batchA-rerun5.txt"
run_fast_gate "$OUT/fast-gate-runner-batchB-rerun5.log" "$OUT/fast-gate-status-batchB-rerun5.txt"
...
{
  echo "fast_gate_batchA=status=$(cut -d= -f2 "$OUT/fast-gate-status-batchA-rerun5.txt" | sed -n '1p');first_failed_step=$(cut -d= -f2 "$OUT/fast-gate-status-batchA-rerun5.txt" | sed -n '2p');failed_command=$(cut -d= -f2- "$OUT/fast-gate-status-batchA-rerun5.txt" | sed -n '3p')"
  echo "fast_gate_batchB=status=$(cut -d= -f2 "$OUT/fast-gate-status-batchB-rerun5.txt" | sed -n '1p');first_failed_step=$(cut -d= -f2 "$OUT/fast-gate-status-batchB-rerun5.txt" | sed -n '2p');failed_command=$(cut -d= -f2- "$OUT/fast-gate-status-batchB-rerun5.txt" | sed -n '3p')"
  echo "prewarm=status=$(cut -d= -f2 "$OUT/prewarm-status-rerun5.txt" | sed -n '1p');first_failed_step=$(cut -d= -f2 "$OUT/prewarm-status-rerun5.txt" | sed -n '2p');failed_command=$(cut -d= -f2- "$OUT/prewarm-status-rerun5.txt" | sed -n '3p')"
  echo "validation=status=$(cut -d= -f2 "$OUT/validation-status-rerun5.txt" | sed -n '1p');first_failed_step=$(cut -d= -f2 "$OUT/validation-status-rerun5.txt" | sed -n '2p');failed_command=$(cut -d= -f2- "$OUT/validation-status-rerun5.txt" | sed -n '3p')"
  echo "step09_retry1=status=$([ "$step09_retry_rc" = "0" ] && echo PASS || echo FAIL);rc=$step09_retry_rc"
} > "$OUT/validation-summary-rerun5.txt"
