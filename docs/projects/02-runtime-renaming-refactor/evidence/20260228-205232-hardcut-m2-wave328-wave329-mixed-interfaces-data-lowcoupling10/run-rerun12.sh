...
run_fast_gate "$OUT/fast-gate-runner-batchA-rerun12.log" "$OUT/fast-gate-status-batchA-rerun12.txt"
run_fast_gate "$OUT/fast-gate-runner-batchB-rerun12.log" "$OUT/fast-gate-status-batchB-rerun12.txt"

VAL_RUNNER="$OUT/validation-runner-rerun12.log"
PRE_STATUS="$OUT/prewarm-status-rerun12.txt"
VAL_STATUS="$OUT/validation-status-rerun12.txt"
: > "$VAL_RUNNER"
...
if [ "$pre_status" = "PASS" ]; then
  for s in STEP01 STEP02 STEP03 STEP04 STEP05 STEP06 STEP07 STEP08 STEP09; do
    ...
    if [ $rc -ne 0 ]; then
      ...
      val_status="FAIL"
      if [ "$s" = "STEP09" ]; then
        RETRY_LOG="$OUT/STEP09-rerun12-retry1.log"
        : > "$RETRY_LOG"
...
{
  echo "fast_gate_batchA=status=$(cut -d= -f2 "$OUT/fast-gate-status-batchA-rerun12.txt" | sed -n '1p');first_failed_step=$(cut -d= -f2 "$OUT/fast-gate-status-batchA-rerun12.txt" | sed -n '2p');failed_command=$(cut -d= -f2- "$OUT/fast-gate-status-batchA-rerun12.txt" | sed -n '3p')"
  echo "fast_gate_batchB=status=$(cut -d= -f2 "$OUT/fast-gate-status-batchB-rerun12.txt" | sed -n '1p');first_failed_step=$(cut -d= -f2 "$OUT/fast-gate-status-batchB-rerun12.txt" | sed -n '2p');failed_command=$(cut -d= -f2- "$OUT/fast-gate-status-batchB-rerun12.txt" | sed -n '3p')"
  echo "prewarm=status=$(cut -d= -f2 "$OUT/prewarm-status-rerun12.txt" | sed -n '1p');first_failed_step=$(cut -d= -f2 "$OUT/prewarm-status-rerun12.txt" | sed -n '2p');failed_command=$(cut -d= -f2- "$OUT/prewarm-status-rerun12.txt" | sed -n '3p')"
  echo "validation=status=$(cut -d= -f2 "$OUT/validation-status-rerun12.txt" | sed -n '1p');first_failed_step=$(cut -d= -f2 "$OUT/validation-status-rerun12.txt" | sed -n '2p');failed_command=$(cut -d= -f2- "$OUT/validation-status-rerun12.txt" | sed -n '3p')"
  echo "step09_retry1=status=$([ "$step09_retry_rc" = "0" ] && echo PASS || echo FAIL);rc=$step09_retry_rc"
} > "$OUT/validation-summary-rerun12.txt"
