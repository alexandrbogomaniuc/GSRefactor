...
OUT="docs/projects/02-runtime-renaming-refactor/evidence/20260228-223946-hardcut-m2-wave334-wave335-persisters-sequencer-stats-lowfanout10"
...
run_fast_gate "$OUT/fast-gate-runner-batchA-rerun2.log" "$OUT/fast-gate-status-batchA-rerun2.txt"
run_fast_gate "$OUT/fast-gate-runner-batchB-rerun2.log" "$OUT/fast-gate-status-batchB-rerun2.txt"

VAL_RUNNER="$OUT/validation-runner-rerun2.log"
PRE_STATUS="$OUT/prewarm-status-rerun2.txt"
VAL_STATUS="$OUT/validation-status-rerun2.txt"
: > "$VAL_RUNNER"
...
if [ "$pre_status" = "PASS" ]; then
  for s in STEP01 STEP02 STEP03 STEP04 STEP05 STEP06 STEP07 STEP08 STEP09; do
    ...
    if [ $rc -ne 0 ]; then
      ...
      val_status="FAIL"
      if [ "$s" = "STEP09" ]; then
        RETRY_LOG="$OUT/STEP09-rerun2-retry1.log"
        : > "$RETRY_LOG"
...
{
  echo "fast_gate_batchA=status=$(cut -d= -f2 "$OUT/fast-gate-status-batchA-rerun2.txt" | sed -n '1p');first_failed_step=$(cut -d= -f2 "$OUT/fast-gate-status-batchA-rerun2.txt" | sed -n '2p');failed_command=$(cut -d= -f2- "$OUT/fast-gate-status-batchA-rerun2.txt" | sed -n '3p')"
  echo "fast_gate_batchB=status=$(cut -d= -f2 "$OUT/fast-gate-status-batchB-rerun2.txt" | sed -n '1p');first_failed_step=$(cut -d= -f2 "$OUT/fast-gate-status-batchB-rerun2.txt" | sed -n '2p');failed_command=$(cut -d= -f2- "$OUT/fast-gate-status-batchB-rerun2.txt" | sed -n '3p')"
  echo "prewarm=status=$(cut -d= -f2 "$OUT/prewarm-status-rerun2.txt" | sed -n '1p');first_failed_step=$(cut -d= -f2 "$OUT/prewarm-status-rerun2.txt" | sed -n '2p');failed_command=$(cut -d= -f2- "$OUT/prewarm-status-rerun2.txt" | sed -n '3p')"
  echo "validation=status=$(cut -d= -f2 "$OUT/validation-status-rerun2.txt" | sed -n '1p');first_failed_step=$(cut -d= -f2 "$OUT/validation-status-rerun2.txt" | sed -n '2p');failed_command=$(cut -d= -f2- "$OUT/validation-status-rerun2.txt" | sed -n '3p')"
  echo "step09_retry1=status=$([ "$step09_retry_rc" = "0" ] && echo PASS || echo FAIL);rc=$step09_retry_rc"
} > "$OUT/validation-summary-rerun2.txt"
