...
## 2026-03-01 10:15 UTC (Hard-cut live batch T: data/enums 10)
- Continued Project 02 hard-cut migration from dirty in-progress workspace using declaration-first low-risk batching.
- Batch intent was `10` declarations; retained after stabilization: `10`.
  - moved: `BonusType`, `BonusGameMode`, `WalletOperationType`, `FRBonusNotification`, `FRBWinOperation`, `FRBonusWin`, `LasthandInfo`, `IdObject`, `IDistributedConfigEntry`, `PromoWinInfo`.
- Bounded compatibility rewires:
...
- Validation snapshot:
  ...
  - canonical runner profile unchanged: `fast_gate_batchA FAIL STEP01`, `fast_gate_batchB FAIL STEP01`, `prewarm FAIL PRE03`, `validation FAIL PRE03`, `STEP09 retry SKIP`.
- Evidence:
  - `docs/projects/02-runtime-renaming-refactor/evidence/20260301-101427-hardcut-live-batchT-data-enums10/`
  - report: `docs/projects/02-runtime-renaming-refactor/247-hard-cut-live-batchT-data-enums10-report-20260301.md`
- Metrics refresh:
...
## 2026-03-01 10:22 UTC (Hard-cut live batch U: kafka/lock/cache-interfaces 13)
- Continued Project 02 hard-cut migration from dirty in-progress workspace using declaration-first low-risk batching.
- Batch intent was `13` declarations; retained after stabilization: `13`.
  - moved: `BasicKafkaResponse`, `VoidKafkaResponse`, `KafkaResponse`, `KafkaRequest`, `KafkaMessage`, `KafkaHandlerException`, `ILockManager`, `LockingInfo`, `IDistributedCache`, `IDistributedCacheEntry`, `Identifiable`, `Pair`, `Triple`.
- Bounded compatibility rewires:
  - package-only declaration migration for retained targets,
  - no blind/global replace,
  - subagent parallel mode attempted but blocked by thread-limit; bounded single-agent fallback used.
- Validation snapshot:
  - focused fast gates: `common FAIL`, `common-wallet FAIL`, `sb-utils FAIL`, `common-gs FAIL`, `common-promo FAIL`.
  - canonical runner profile: `fast_gate_batchA FAIL STEP01`, `fast_gate_batchB FAIL STEP01`, `prewarm FAIL PRE02`, `validation FAIL PRE02`, `STEP09 retry SKIP`.
- Evidence:
  - `docs/projects/02-runtime-renaming-refactor/evidence/20260301-102006-hardcut-live-batchU-kafka-lock-cacheiface13/`
  - report: `docs/projects/02-runtime-renaming-refactor/248-hard-cut-live-batchU-kafka-lock-cacheiface13-report-20260301.md`
- Metrics refresh:
...
## 2026-03-01 10:37 UTC (Hard-cut live batch Y: residual Java 12)
- Completed residual Java declaration migration in previously dirty files.
- Batch intent was `12` declarations; retained: `12`.
- Validation snapshot:
  - focused fast gates: `common FAIL`, `common-wallet FAIL`, `sb-utils FAIL`, `common-gs FAIL`, `common-promo FAIL`.
  - canonical runner profile: `fast_gate_batchA FAIL STEP01`, `fast_gate_batchB FAIL STEP01`, `prewarm FAIL PRE01`, `validation FAIL PRE01`, `STEP09 retry SKIP`.
- Evidence:
  - `docs/projects/02-runtime-renaming-refactor/evidence/20260301-103720-hardcut-live-batchY-residual-java12/`
  - report: `docs/projects/02-runtime-renaming-refactor/252-hard-cut-live-batchY-residual-java12-report-20260301.md`
- Metrics refresh:
...
## 2026-03-02 18:05 UTC (Hard-cut live Batch BB+BC)
- Continued Project 02 stabilization with integrated live batches `BB+BC` in `/Users/alexb/Documents/Dev/Dev_new`.
- Batch content:
  - BB (`1` rewire): normalized `PlayerRoundInfo` `Pair` import (`com.dgphoenix -> com.abs`).
  - BC (`2` rewires): normalized `EnemyGame` + `PlayGameState` `CommonException` imports (`com.dgphoenix -> com.abs`).
  - retained total: `3` import/signature-boundary rewires across `3` files.
- Validation:
  - targeted gates: `common-games` PASS, `web` FAIL, `clashofthegods` PASS.
  - `web` first-fail narrowed to residual pirates boundary lanes:
    - `EnemyGame` unreported `com.dgphoenix...CommonException` at dependent call boundary,
    - `PlayGameState` Pair/Triple incompatibilities at remaining mixed-namespace call sites.
  - canonical matrix profile: `fast_gate_batchA FAIL STEP01`, `fast_gate_batchB FAIL STEP01`, `prewarm FAIL PRE01`, `validation FAIL PRE01`, `STEP09 retry SKIP`.
  - canonical failures remain environment-level Maven dependency resolution in current sandbox (external repo/DNS), not batch-local logic regression.
- Evidence:
  ...
  - report: `/Users/alexb/Documents/Dev/Dev_new/docs/projects/02-runtime-renaming-refactor/271-hard-cut-live-batchBBBC-pirates-boundary-import-rewire3-report-20260302.md`
- Measured movement:
...
## 2026-03-03 04:02 UTC (Hard-cut live Batch CH)
- Continued Project 02 stabilization with live batch `CH` in `/Users/alexb/Documents/Dev/Dev_new`.
- Batch content:
  - CH (`1` rewire): `GetBattlegroundStartGameUrlHandler` boundary harmonization for `getRoomWithoutCreation(...)` with legacy-to-ABS exception bridge.
  - retained total: `1` targeted rewire across `1` file.
- Validation:
  - targeted gates: `common-games` PASS, `bots` PASS, `web` FAIL, `clashofthegods` compile gate FAIL in known lane.
  - `web` first-fail shifted off `GetBattlegroundStartGameUrlHandler` to localized `BotOpenRoomHandler` boundary (`[88,55]`).
  - canonical matrix profile: `fast_gate_batchA FAIL STEP09`, `fast_gate_batchB FAIL STEP09`, `prewarm PASS`, `validation FAIL STEP09`, `STEP09 retry1 FAIL rc=2`.
  - canonical failures remain in known smoke-stage external/runtime lane, not CH import-local semantics.
- Evidence:
  ...
  - report: `/Users/alexb/Documents/Dev/Dev_new/docs/projects/02-runtime-renaming-refactor/288-hard-cut-live-batchCH-bgbstarturl-exception-bridge-rewire2-report-20260303.md`
- Measured movement:
...
- Batch content:
  - `CW` (`9` edits): added `soak` command in `refactor-onboard.mjs` with artifacted repeated-smoke summaries and rc classification (`0/2/3`).
  - `CX` (`8` edits): added strict-fail diagnostics in `refactor-start.sh` (`REFACTOR_DIAG_ON_FAIL`, `REFACTOR_DIAG_TAIL_LINES`, compose/ nginx/ probe hints) and restart-count warn surface; README alignment.
- Validation outcomes:
  - targeted fast gates: `common-games PASS`, `bots PASS`, `web PASS`, `clashofthegods PASS`.
  - full matrix profile: `fast_gate_batchA PASS`, `fast_gate_batchB PASS`, `prewarm PASS`, `validation PASS`, `STEP09 retry1 SKIP`.
  - additional soak signoff: two independent soak cycles (`2 runs each`) both all-pass (`final_rc=0`).
- Evidence/report anchor:
...
- Executed post-signoff observation checkpoint (`Batch CZ`) in `/Users/alexb/Documents/Dev/Dev_new`.
- Validation outcomes:
  - full matrix rerun: PASS across fast gates, prewarm, and validation (`STEP09 retry skipped`).
  - soak cycle 1 (`2` runs): all-pass (`final_rc=0`).
  - soak cycle 2 (`2` runs): all-pass (`final_rc=0`).
- Evidence/report anchor:
